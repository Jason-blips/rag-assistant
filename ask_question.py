...
def _summarize_with_llm(question: str, docs, model: str) -> str:
    ...
    system_prompt = (
        "你是一个严谨且实用的智能问答助手。"
        "请优先依据给定资料回答问题；如果资料不足，可补充通用知识，但不要编造事实。"
        "输出简洁、直接、可执行，不要使用模板化标题。"
    )
    if not has_docs:
        user_prompt = f"""下面是用户问题：
{question}

当前没有检索到可用课件内容。请直接给出一个清晰、可执行的回答：
- 如果用户要代码，请直接给出可运行示例（必要时附简短说明）。
- 如果涉及实时数据，先说明你无法直接获取实时值，再给出可执行的查询路径或示例。

请直接输出“中文最终回答”正文本身，不要加编号，不要加“关键依据”小节。
"""
    else:
...
请直接输出最终回答正文，不要使用“中文最终回答/英文标准答案/关键依据”等标题或编号。
"""
